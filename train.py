"""
train_soil_enhanced.py

Enhanced version for Soil Condition training only
...
def train_model_enhanced(X_train, X_test, y_train, y_test, model_name, 
    ...
    k_features = min(int(n_features * 0.6), 70)  # Select 60% of features, max 70
...
def main(args):
    print("="*60)
    print("ENHANCED SOIL CONDITION TRAINING")
    print("="*60)
    ...
    # Detect target
    soil_col = detect_target_column(df)
    print(f"\nDetected Soil Condition column: '{soil_col}'")
    
    if not soil_col:
        raise ValueError("Could not detect Soil Condition target column.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Enhanced soil condition training')
    parser.add_argument('--train_csv', required=True, help='Training CSV path')
    parser.add_argument('--model_out', default='enhanced_soil_model.pkl', help='Model output path')
